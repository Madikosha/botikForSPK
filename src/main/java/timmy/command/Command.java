...
public abstract class Command {
    ...
    protected void sendMessageDoc(String value_1, String value_2, String type) throws TelegramApiException {
            if (type.equals("p")){
                bot.execute(new SendPhoto().setChatId(chatId).setPhoto(propertiesDao.getPropertiesValueLangId(value_1)));
                bot.execute(new SendPhoto().setChatId(chatId).setPhoto(propertiesDao.getPropertiesValueLangId(value_2)));
            } else if (type.equals("d")){
                bot.execute(new SendDocument().setChatId(chatId).setDocument(propertiesDao.getPropertiesValueLangId(value_1)));
                bot.execute(new SendDocument().setChatId(chatId).setDocument(propertiesDao.getPropertiesValueLangId(value_2)));
            }
    }
    protected void sendMessageOneDoc(String value_1, String type) throws TelegramApiException {
        if (type.equals("p")){
            bot.execute(new SendPhoto().setChatId(chatId).setPhoto(propertiesDao.getPropertiesValueLangId(value_1)));
        } else if (type.equals("d")){
            bot.execute(new SendDocument().setChatId(chatId).setDocument(propertiesDao.getPropertiesValueLangId(value_1)));
