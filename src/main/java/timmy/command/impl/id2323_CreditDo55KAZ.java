...
@Slf4j
public class id2323_CreditDo55KAZ extends Command {
    private int credit;
    private int percent;
    private int months;
    private double res;
...
//                    System.out.println(percent);
    ...
    private void        sendReport() throws TelegramApiException {
        ...
        Sheet sheets        = wb.createSheet("Кредиттiк калькулятор");
        // -------------------------Стиль ячеек-------------------------
...
//        System.out.println(sum);
        ...
        sheets  .createRow(rowIndex6) .createCell(CellIndex6)  .setCellValue("Кредит сомасы");
        sheet   .getRow(rowIndex6)    .getCell(CellIndex6)     .setCellStyle(styleTitle);
        sheets  .createRow(rowIndex2)    .createCell(CellIndex2).setCellValue("Кредит мерзімі");
        sheet   .getRow(rowIndex2)    .getCell(CellIndex2)     .setCellStyle(styleTitle);
        sheets  .createRow(rowIndex3)    .createCell(CellIndex3).setCellValue("Пайыздық мөлшерлеме");
        sheet   .getRow(rowIndex3)    .getCell(CellIndex3)     .setCellStyle(styleTitle);
        sheets  .createRow(rowIndex4)    .createCell(CellIndex4).setCellValue("Төлемдердің жалпы сомасы");
        sheet   .getRow(rowIndex4)    .getCell(CellIndex4)     .setCellStyle(styleTitle);
        sheets  .createRow(rowIndex5)    .createCell(CellIndex5).setCellValue("Жалпы сомасы %");
        ...
        int rowIndex    = 7;
        int CellIndex   = 0;
        sheets  .createRow(rowIndex) .createCell(CellIndex)  .setCellValue("Ай");
        sheet   .getRow(rowIndex)    .getCell(CellIndex)     .setCellStyle(styleTitle);
        sheets  .getRow(rowIndex)    .createCell(++CellIndex).setCellValue("Негізгі борыш");
        sheet   .getRow(rowIndex)    .getCell(CellIndex)     .setCellStyle(styleTitle);
        sheets  .getRow(rowIndex)    .createCell(++CellIndex).setCellValue("Есептелген %");
        sheet   .getRow(rowIndex)    .getCell(CellIndex)     .setCellStyle(styleTitle);
        sheets  .getRow(rowIndex)    .createCell(++CellIndex).setCellValue("Негізгі борыш сомасы");
        sheet   .getRow(rowIndex)    .getCell(CellIndex)     .setCellStyle(styleTitle);
        sheets  .getRow(rowIndex)    .createCell(++CellIndex).setCellValue("Төлем");
        sheet   .getRow(rowIndex)    .getCell(CellIndex)     .setCellStyle(styleTitle);
        ...
        String filename = String.format("Алдын ала төлем кестесі.xlsx", DateUtil.getDayDate(new Date()));
