...
public class UserDao extends AbstractDao<User>{
    ...
    public void         update(User user) {
        sql = "UPDATE USERS SET PHONE = ?, FULL_NAME = ?, EMAIL = ?, USER_NAME = ?, COMPANY = ?, DEPARTMENT = ? WHERE CHAT_ID = ?";
        getJdbcTemplate().update(sql, user.getPhone(), user.getFullName(), user.getEmail(), user.getUserName(), user.getCompany(), user.getDepartment(), user.getChatId());
    }
